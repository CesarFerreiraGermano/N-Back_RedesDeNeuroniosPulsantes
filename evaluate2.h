...
#define JANELAS 50 // Quantidade de janelas (nº de vezes em que inicia-se um novo estímulo)
...
float evaluate (int n){
	...
	float TP, TN, FP, FN, // Categorias da matriz de confusão
		  ...
		  TNR, // True Negative Rate ou Especificidade: Proporção de TN em relação à soma de TN + FP
	...
	float ROC[2][trials+1];
	//ROC[0][0] = ROC[1][0] = 0;
	ROC[0][20] = ROC[1][20] = 1;

	float y1,y2,x1,x2,coefA,coefB,valorY,area;
	...
	for (int k=0;k<trials;k++){
		...
		for (int iterations=0; iterations<=(JANELAS*100); iterations++){
            ...
            if(iterations%100==0){ // Início de nova janela
				...
				atual = rand()%4; // Novo N é calculado aleatoriamente | [0,3]
