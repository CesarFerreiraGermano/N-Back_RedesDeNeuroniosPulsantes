...
class Genetico{
	...
	geracoesPrevistas = 5000;
        taxa_cruzamento=0.7;
        eletismo=0; //Com eletismo
        controle_mutacao=2; //Tipo de mutacao 1: fixa; 2:deterministica
        taxa_mutacao=0.1;
        porcentagem_mutacao=0.01958; /*Indica a porcentagem de mutacao Ex. 10% para mais ou para menos. Se controle_mutacao=2, entao a porcetagem de mutacao cai
    ...
    void write_pop(int best)
    {
	    ...
	    { FILE * popfile = fopen("logPopulation.txt","w");
		    for (int i=0;i<POP;i++){
		        for (int j=0;j<LEN;j++){
			         fprintf(popfile,"%.15lf ",gene[i][j]); // 30 casas
			      }
	         fprintf(popfile,"\n");
	      }
		    fclose(popfile);
